...
So we're now going to talk about AWS components a little bit more, before we get to the lab. I just want to make the point that I do think that some of the cloud deployment world is a bit like learning biology. I think of working with LMS. The stuff that is my greatest forte, I feel, is more like physics—it’s more about understanding, experimenting, learning about how things work and why they work. With AWS, there’s a lot of learning, a lot of anatomy, and a lot of words and acronyms to memorize if you want to feel fluent in it. I was never much good at biology at school. I’m sure some of you are brilliant at biology. I find it hard work, and some of you may find it hard work as well. I find learning about LMS simpler. With AWS, as I say, there’s going to be lots of terminology, and my advice to you is to not let it get to you. Just listen to all these words, copy what I do, and expect that it will sink in over time as you start to have S3, Lambda, CloudFront, and all these other things just roll off the tongue. But we are now going to have a biology lesson as we get into the AWS components that we'll be using this week.

So I’m going to talk about five of them, which are the main five we will use: Amazon S3, Lambda, CloudFront, API Gateway, and Amazon Bedrock. First, Amazon S3, which stands for Simple Storage Service. This is very simple indeed. It is just like having a shared drive, like a Google Drive in the cloud, that you can access for your software. It’s organized into things called buckets. When you hear people saying “S3 bucket,” they are referring to one particular shared drive with a specific name that you can access. In that bucket, you can have directories and files, just like any shared drive. That is S3. Of course, there’s lots of detail and complexity around it, but that’s the simple version.

Next is Lambda, which we’ve already talked about. You’re going to get to love it. Lambda is about individual functions running on the cloud, and you only pay for the CPU clock cycles that you use.

Then there’s CloudFront. What is CloudFront? Have you heard of something called a CDN—a Content Delivery Network? CDNs are part of how the internet works when it comes to larger files like images and JavaScript files. It would be very inefficient if every time you looked at a web page, like a news web page, it went all the way back to the main servers to fetch all images, traveling long distances across the internet. The idea behind CDNs is that for larger static files, like images, it’s more efficient for companies to push them out to smaller data centers all over the world. That way, when a user brings up a web page, it loads some of the content from the actual server, but images and other assets are collected from a local data center near the user, making it much faster. A service that allows companies to do this is known as a CDN, and Amazon’s offering is called CloudFront. When you work with CloudFront, it’s called making a CloudFront distribution. This means distributing the assets associated with your app all over the world, so when people bring up a web page, some content is fetched from Amazon servers while other content is fetched from CloudFront servers closer to the user. If you didn’t get all of that, it doesn’t matter—we’ll be playing with it, and you’ll get a better sense for it later. But hopefully, you now know three of the Amazon words we’ll be dealing with today.

The next component is Amazon API Gateway. This is where you can manage external APIs that you offer to the world, which might call Lambda functions, and you can add instrumentation such as rate limiting and scaling management. Lambda can be accessed directly from the outside world, but the best practice is to set up an API Gateway, giving you more control and making your application more enterprise-ready. That’s what we’ll be using this week, though we won’t get hands-on with it today.

Finally, we have Amazon Bedrock. This is one of Amazon’s two LLM offerings, and it allows you to quickly build generative AI apps by connecting to foundation models through Bedrock. It has additional features, like an agent platform, that we’ll look at later. A minor point to note is the branding difference: some products are branded “Amazon” and some “AWS.” Amazon branding is typically used for early offerings, like S3, or consumer-facing products like Bedrock. Nerdier offerings, aimed more at engineers, are branded AWS, like AWS Lambda. If you want to be precise with the terminology, it’s worth following the documentation’s naming conventions.

# **D) Day 1 - Building Your Digital Twin: AWS Lambda + Bedrock Architecture Setup**

And now let me unveil to you the project we'll be working on throughout this week. The digital twin—those of you who took my Agentic course will have already built a digital twin, which I called your Career Alter Ego in that course. Essentially, it’s a chatbot that represents you to the outside world. It’s like the future of the resume. Instead of having a static resume on your website, you have a chatbot that knows about you, your career history, and your interests. Recruiters or managers can interact with it to ask questions about you and what you might be interested in. You can prep it and improve its responses over time to make it better at answering the kinds of questions people are likely to ask.

Now I’m calling this digital twin Mark Two, because it represents a significant step forward from what I did in the Agentic course. Some parts of the previous functionality, such as tool use, will not be included in this version; instead, they will be part of assignments for those who took the Agentic course. But don’t worry if you haven’t taken the course—you don’t need a Mark One. We will also be starting from scratch with this new version.

So what is this going to do? Let me explain the deployment architecture of the digital twin. It might look a bit unwieldy if you haven’t seen these sorts of setups before. There will be a bunch of boxes representing different AWS components. But rest assured, however complex this looks, it’s actually far simpler than what’s coming next week—this is going to be the easy part.

The digital twin begins with Lambda functions that we will write. One Lambda function will handle the business logic of the digital twin. This function will respond to requests from hiring managers who want to chat with your digital twin about your background, accomplishments, or career interests. Since every call to a large language model (LLM) is stateless, meaning it has no knowledge of previous interactions, we need to store the conversation context. To achieve this, we’ll use an S3 bucket called “memory,” with separate files for each user conversation. The Lambda function will access this bucket to maintain conversation context.

We will also build an API Gateway that allows external users to call our Lambda function, enabling conversations with the LLM. These components—the Lambda function, the S3 memory bucket, and API Gateway—make up the backend of the system.

Next, let’s talk about the frontend. We’ve built a Next.js app, and one of the things we can do with it is export a static website that represents the frontend. This static website, which consists of HTML, JavaScript, and CSS generated from TypeScript and Tailwind, will be deployed to another S3 bucket called “frontend.” To make it globally accessible and fast, we will use CloudFront, which distributes the website’s content across data centers worldwide.

So now we have a frontend and a backend. How do these two components come together? The connection happens in the user’s browser. When a user enters the URL of our CloudFront distribution, their browser will fetch and render the static site, displaying the chat interface. When the user submits a message, the browser makes an API call to the API Gateway, which triggers the Lambda function. The Lambda function then interacts with the S3 memory bucket to maintain conversation context and communicates with the LLM to generate responses.

We haven’t yet mentioned the LLM itself. The Lambda business logic will make calls to the LLM through Amazon Bedrock, which is AWS’s service for managing LLM interactions. This is represented in purple in our architecture diagram, as purple indicates AI components. And with that, the deployment architecture for the digital twin Mark Two is complete.

Before we go to the lab, a few notes. This week will be somewhat tiresome, as it’s primarily focused on environment setup. It will involve lots of copying and pasting, and it won’t feel like traditional coding labs where you inspect notebooks line by line. This process is essential for building familiarity with AWS components, and you may need to repeat some steps to reinforce your understanding.

Read the guides carefully, as they are updated regularly to help troubleshoot issues. Keep an eye on community contributions for extra tips. Use the LMS, Cursor, or Cloud Code as needed. When using agents or LLMs for help, first make sure they understand the context by showing them the guides you’ve been following. Always validate their suggestions, as they often hallucinate or offer quick fixes that might not be robust. Treat them as eager junior assistants who can do research but still require your critical oversight.

Finally, remember that I’m here for questions and usually respond quickly unless I’m traveling or sleeping. This setup is harder than previous courses, so focus on developing strategies for debugging. AWS also offers paid support if needed. The most important factor throughout this process is patience—environment setup can feel repetitive and frustrating, but once it comes together, it’s highly satisfying.

With that, let’s go to the lab. Today, we’ll focus on building the digital twin locally. We won’t deploy it on AWS yet, but by the end of today, it will be ready for full deployment tomorrow.

# **E) Day 1 - Building Your AI Digital Twin: Production Setup with NextJS App Router**

And welcome back to Cursor. I am looking in Cursor at the production project. To get started, go to File → New Window and select “Production” or “Open Production.” Remember to always do a git pull as explained in the guides to make sure you have the latest code. In the production repository, there is a “Week Two” folder with five days’ worth of guides. If you right-click on Day One and select “Open Preview,” it will open in a nicely formatted view.

Today, we are introducing the twin. We will be building an AI digital twin—a conversational AI that represents you to future employers. Today, we’ll start by building a local version, focusing particularly on ensuring that memory works. This allows for an apparently stateful conversation with an LLM, using the technique of passing in the conversation so far.

One thing that will be very different today is that we’ll be using the App Router mode of Next.js, rather than the older Pages Router. The Pages Router is simple: there is a directory called pages, and every file in that directory becomes a root URL accessible to the outside world. For example, index.tsx represents the main root, while product.tsx corresponds to /product.

The App Router is more suitable for robust applications. In it, there is an app directory, and a file called page.tsx represents the root URL. Each additional page requires a subdirectory inside app. For example, a page /about would need an about subdirectory, and a page.tsx inside it would serve that route. In short, App Router uses subdirectories for pages, whereas Pages Router uses individual TypeScript files. This week, we’ll be using App Router, since we aren’t deploying with Clark.

Now it’s time to set up our project for the week—the twin project. First, open Cursor (or any IDE you like) and create a new project called twin. In File → Open Folder, navigate to your projects directory, create a new folder called twin, and open it. You now have a completely empty canvas for this project. We will build everything in this directory from scratch; this will be your personal repo.

Next, open a terminal. For convenience, we can copy the contents of the Week Two folder from the production repo. This ensures that all five days’ guides are available locally, so you don’t have to navigate back and forth. You can copy the folder by command line or drag-and-drop—whichever you prefer. Open the preview for Day One again, and you’re ready to start.

Step two: create project directories. In File Explorer, right-click and create a folder called back_end for our backend code. Then create another folder called memory—this is where local memory files will reside, which later will be stored in S3 when deployed.

Step three: create the frontend. Since we need a frontend folder to match the backend, we’ll use create-next-app. Open a terminal and run npx create-next-app front_end with the TypeScript variant and Tailwind. Say “no” to using a source directory and TurboPack. Once complete, you’ll see a folder structure with an app folder, confirming that we are using App Router. Now we have a back_end, front_end, memory, and week_two (guides) folders. Verify your setup, and if it looks like this, we’re ready to move on.

Part two: install a Python package manager. We’ll use pipx (referred to as UV), which is robust and fast, written in Rust. UV is an alternative to Anaconda or direct virtual environments, allowing you to manage dependencies efficiently. Follow the installation instructions provided in the guides. You can verify your installation with uv --version and update it with uv self-update if needed.

Part three: create a requirements.txt file in the back_end directory to install the necessary Python packages. Be careful to place this file in the correct directory to avoid issues later. Copy the package list from the guide, paste it into requirements.txt, and save.

Next, create an environment configuration file. In back_end, create a .env file. This file will contain two key entries: your OpenAI API key (OPENAI_API_KEY) and CORS_ORIGINS. Be precise with spelling—CORS is critical for security controls on which frontend or backend connections are allowed. Copy the value for CORS from the guide. Then find your OpenAI API key, paste it into .env, and save. If you see a white blob or stop symbol, don’t worry—this indicates AI features are disabled in Cursor for security. The key is to ensure the file is saved.

With that, your environment setup for the twin project is complete. You now have your backend, frontend, memory, guides, and Python environment ready for local development and testing.
...
We’re about halfway through our setup, so if things feel overwhelming, don’t worry—you can always come back and repeat steps if needed. The next task is to create another S3 bucket. Remember, we have two S3 buckets: one for memory and another for the entire static front-end site. The new bucket will be called twin-front-end, appended with a random ID, and it needs to be in the same region as the previous bucket. While creating this bucket, make sure to uncheck the “Block all public access” box, because we want this bucket to be publicly accessible for our website. Once we name it, copy the account ID into the bucket name, acknowledge that the bucket will be public, and then click Create bucket. After successful creation, we can move on to configuring static website hosting.

Next, we enable static website hosting on the front-end bucket. Navigate to the bucket properties and locate “Static website hosting.” Enable it and set index.html as the index document and 404.html as the error document. Save these changes. This ensures that our S3 bucket can serve as a static website.

Once the static hosting is set, the next step involves configuring permissions. Go to the Permissions tab and edit the Bucket Policy. We need to add a policy that allows public access to the bucket, enabling the world to access the website. When pasting the policy, ensure that the bucket name and the Amazon Resource Number (ARN) are correctly included, ending with /* to cover all objects. Only after making these edits, click Save changes. This step is finicky, and getting it wrong will prevent the website from being publicly accessible, but this manual approach will work for now. Later, when using Terraform, all of this can be automated via code.

With our S3 buckets ready, we move on to creating the API Gateway, which acts as a bridge between the outside world and our Lambda function. You could expose Lambda directly, but API Gateway is a more robust, enterprise-level solution. In the AWS console, search for API Gateway and select HTTP API. Name it twin API gateway, and then Add Integrations. Choose the Lambda type, ensure it’s in the correct region, and select the Lambda function (likely twin API) for integration.

Next, we configure routes. Start with a default route using the ANY method, targeting the root path, and integrate it with our Lambda function. Then add routes for GET /, GET /health, POST /chat, and OPTIONS /. Each route should point to the twin API Lambda function. The OPTIONS route is especially important for CORS, allowing the browser and server to negotiate permissions.

After routes, configure the stages with default settings, review the configuration, and create the API Gateway. Once created, configure CORS settings. In the API Gateway, navigate to CORS, press configure, and add * for origins, headers, and methods. Set the max age to 300 seconds. Make sure to press the Add button where required; otherwise, the settings won’t save. Save these changes.

Finally, it’s time to test the setup. Navigate to the API Gateway, copy the endpoint, and open it in a browser with /health appended. If everything is configured correctly, the response should be:

Healthy. Use S3: True

This confirms that the request went to the Amazon endpoint, which passed control to the Lambda function. The Lambda function spawned a FastAPI server and processed the /health route, returning the expected response. Depending on the uploaded Lambda zip file, you might see additional information such as Bedrock models, but the essential health check confirms that the infrastructure works.

In summary, we now have two S3 buckets, a Lambda function, and an API Gateway fully integrated. The S3 bucket hosts our front-end, while the API Gateway exposes the Lambda function securely to the outside world. This sets the foundation for a fully operational, serverless web application, bringing us into the final stages of deployment.

# **M) Day 2 - Deploying AI Frontend Through CloudFront for Global Distribution**

For the next step, start by going back to the API Gateway and copying the URL for the Twitter API Gateway. This endpoint is what our front end will use to communicate with the back end. Once you have it copied, we move on to updating the front-end code. In the components/twin.tsx file, locate the line of code that currently calls localhost:8000—this is the placeholder for the back-end service. Replace it with the actual API Gateway endpoint, specifically targeting the /chat route. Ensure the URL is correctly formatted, as this is where the front-end will post requests to the back-end Lambda function.

Next, we need to update the Next.js configuration to allow static exports. Open the next.config.js file, which may initially be empty, and add the necessary export configuration. This ensures that Next.js can generate a fully static version of our site, which is crucial for hosting on S3.

With the configuration updated, we can now build a static export of the website. Open a terminal in the front-end directory and run npm run build. This will create an optimized production build and generate an out directory containing all the static files—HTML, JavaScript, and CSS—that comprise the website. These files are now ready to be hosted on S3, and you’ll notice that the index.html and 404.html files are included, corresponding to the setup we configured earlier in the front-end bucket.

The next step is uploading the static site to S3. In the terminal, run the command aws s3 sync out/ s3://<bucket-name>/ --delete, replacing <bucket-name> with your full front-end bucket name, including the account ID. This command synchronizes the out directory with your S3 bucket and removes any files in the bucket that no longer exist in the build directory. Once the command completes, the static site is fully uploaded.

Now it’s time to test the front end. Go to your S3 bucket, navigate to the properties, and locate the static website hosting URL. Open that URL in a browser. If everything is configured correctly, the site will load, and you should see the front-end interface. When you interact with the site, it sends requests to the back-end API, which responds via the Lambda function, confirming that the front end and back end are working together. While this test works directly via the S3 endpoint, the eventual goal is to serve the site through CloudFront for improved performance and global distribution.

To proceed with CloudFront, first copy the static website endpoint from the S3 front-end bucket. Then, go to CloudFront in the AWS console and create a new distribution called Twin Distribution. When specifying the origin, select Custom Origin instead of S3, and paste the S3 static site endpoint without the http:// prefix. It is crucial to configure the origin settings to HTTP only, as CloudFront needs to interact with the S3 static site over HTTP. Other settings can remain default.

During setup, you may be prompted to enable a Web Application Firewall (WAF) for extra protection against attacks like DDoS. This is optional and can be skipped unless your organization requires it. Once everything is configured, click Create distribution. CloudFront will begin deploying the distribution globally, which can take 5 to 10 minutes. During this time, the status will show as "Deploying." When deployment completes and the status changes to "Enabled," your CloudFront distribution is ready, and the static site can now be accessed via the CloudFront endpoint, providing faster delivery and better global reach.

# **N) Day 2 - Testing Your Live AI Agent and Configuring CORS for Production**

At this stage, we are almost at the finish line. The front-end has been built and deployed, and our CloudFront distribution is either still deploying or has already finished. The final configuration step involves updating the CORS settings on the back end. Currently, the Lambda function’s CORS origin is set to *, which means anyone could make requests to the API from anywhere. This is insecure, so we need to restrict it to only allow requests coming from our CloudFront distribution.

Once the CloudFront distribution has deployed successfully, copy its URL. Then go back to the Lambda function configuration, navigate to Environment Variables, and update the CORS variable to include the CloudFront URL. Be sure to include the full URL with https:// at the beginning. This tells the backend which origins are allowed to make requests, preventing CORS errors. Once this environment variable is updated, everything should be ready for testing.

Now comes the moment of truth: testing the full deployment. Navigate to the CloudFront distribution URL in a new browser tab. This is where the front-end is now globally accessible. The front end connects to the back end through the API Gateway and Lambda function. When you interact with the site, you should see responses from your digital twin, demonstrating that the front end and back end are fully integrated. For example, asking “Hi there” or “Do you like cheese?” produces live responses based on the logic in the Lambda function. This confirms that the system is working end-to-end, with requests routed from the browser to CloudFront, through API Gateway, into Lambda, and then processed by the application code.

Additionally, you can check the memory S3 bucket. This bucket stores the JSON objects representing the conversation history. For instance, after interacting with the digital twin, you’ll find a JSON object corresponding to your recent chat, including questions like the cheese question and the responses. This verifies that the memory functionality is operational and properly integrated with the architecture.
